...
namespace JSLib {
	...
	inline const Disposable<Matrix> operator+(const Matrix& m1, const Matrix& m2)
	{
		...
		std::transform(m1.begin(), m1.end(), m2.begin(), m2.end(), std::plus<double>());
		return temp;
	}
	...
	{
		Matrix temp(m1.rows(), m1.columns());
		std::transform(m1.begin(), m1.end(), m2.begin(), m2.end(), std::minus<double>());
		return temp;
	}

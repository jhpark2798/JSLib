...
namespace JSLib {
	...
	public:
		...
		public:
			...
			virtual Disposable<Array> drift(const StochasticProcess&, 
				double t0, const Array& x0, double dt) const = 0; 
			virtual Disposable<Matrix> diffusion(const StochasticProcess&, 
				double t0, const Array& x0, double dt) const = 0;
		...
		virtual size_t size() const = 0;	// process의 갯수
		virtual size_t factors() const; // brownian motion의 갯수
		...
		//t0에서 dt 사이의 process의 expectation
		virtual Disposable<Array> expectation(double t0, const Array& x0, double dt) const;
		//t0에서 dt 사이의 process의 std
		virtual Disposable<Matrix> stdDeviation(double t0, const Array& x0, double dt) const;
		//t0에서 dt 사이의 process의 covariance
		virtual Disposable<Matrix> covariance(double t0, const Array& x0, double dt) const;
		//t0에서 dt 경과 후 process의 value
		virtual Disposable<Array> evolve(double t0, 
			const Array& x0, double dt, const Array& dw) const;
		//process의 x0에 dx를 더한 값을 반환
	...
	public:
		...
		public:
			virtual ~discretization() {}
			virtual double drift(const StochasticProcess1D&, 
				double t0, double x0, double dt) const = 0;
			virtual double diffusion(const StochasticProcess1D&, 
				double t0, double x0, double dt) const = 0;
			virtual double variance(const StochasticProcess1D&, 
				double t0, double x0, double dt) const = 0;
		};
	...
	private:
		...
		Disposable<Array> evolve(double t0, 
			const Array& x0, double dt, const Array& dw) const;
